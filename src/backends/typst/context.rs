...
//! and `.brand` to layouts as plain dictionaries. Registered as the
...
pub fn build_context_source(meta: &DocMeta, brand: &BrandSpec) -> String {
...
mod tests {
    ...
    fn empty_meta_and_brand_produce_valid_defaults() {
        let out = build_context_source(&DocMeta::default(), &BrandSpec::default());
    ...
    fn extra_keys_flatten_into_doc_meta() {
        ...
        let out = build_context_source(&meta, &BrandSpec::default());
    ...
    fn brand_palette_and_fonts_are_nested_dicts() {
        ...
        let out = build_context_source(&DocMeta::default(), &brand);
